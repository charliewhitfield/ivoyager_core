...
## File name for the input-map cache file.
...
## Underlying cache handler that backs all of this manager's persistence
## methods.
var cache_handler: IVCacheHandler
## Current input-map state, keyed by action name.
var current: Dictionary[StringName, Variant] = {}

## Scancodes the user cannot overwrite (with or without key modifiers).
var reserved_scancodes: Array[int] = [] # user can't overwrite w/ or w/out key mods
## Map of supported event-class name to its [Object] type. Extend as needed.
...
## Reverse lookup from scancode-with-modifiers to the bound action name.
...
## Returns the array index in [member current] of the [param index]-th event of
## class [param event_class] bound to [param action]. [param index] may be
## arbitrarily large; if past the last matching entry, the size of the events
## array is returned.
...
## Returns all event dictionaries of class [param event_class] currently bound
## to [param action].
...
## Removes the [param index]-th event of class [param event_class] from
## [param action]. If [param suppress_caching] == true, call
## [method cache_now] later.
...
## Writes the current input map to the cache file. Useful after a batch of
## modifications made with [param suppress_caching] = true.
func cache_now() -> void:
	cache_handler.cache_now()


## Returns true if action [param key] currently equals its default binding.
func is_default(key: StringName) -> bool:
	return cache_handler.is_default(key)


## Returns true if [i]all[/i] actions currently equal their defaults.
...
## Returns true if the in-memory bindings match the cache file (i.e., no
## un-cached changes are pending).
func is_cache_current() -> bool:
	return cache_handler.is_cache_current()


## Reloads bindings from the cache file, discarding any in-memory changes.
...
## Use this in place of [method InputEvent.is_action_pressed] for any action
## defined here: none of them exist until [IVCoreInitializer] instantiates this
## class, and a project is free to remove or rename any of them.
static func is_action_pressed(event: InputEvent, action: StringName, allow_echo := false,
...
static func is_action_released(event: InputEvent, action: StringName, exact_match := false) -> bool:
	return InputMap.has_action(action) and event.is_action_released(action, exact_match)


## Returns scancodes (with modifier bits OR'd in) for every keyboard event
## bound to [param action].
...
## Returns [param keycode] with all modifier-key bits cleared.
