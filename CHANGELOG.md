...
2. The shader warm-up is opt-in; see IVShaderWarmup addition below. This forces shader compilation under the boot or splash screen rather than mid-flight when visiting a body for the first time at runtime. These times can be significant, especially for Compatibility renderer; see [SHADER_COMPILE_PROFILING.md](SHADER_COMPILE_PROFILING.md).
...
* [shader/gdshaderinc breaking] [Project breaking] User Option "Atmosphere Quality" (setting `atmosphere_quality`, new `IVGraphicsManager.atmosphere_quality_settings`): Normal, or Reduced, which runs a shorter along-ray quadrature and caps the beyond-limb ring's taps for a large saving on weak hardware, moving nothing outside a band a few pixels wide at the limb. Both tiers are one shader program -- `_atmosphere.gdshaderinc` packs both Gauss-Legendre rules into one table and three new shader globals (`iv_atm_gl_first`, `iv_atm_gl_nodes`, `iv_atm_ring_max_taps`) select between them -- so a tier change costs no compile, which is what makes this a live setting rather than the restart one [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md) expected. Those globals replace the `atm_gl6_nodes` and `atm_ring_max_taps` uniforms and the `ATM_GL6_T` / `ATM_GL6_W` tables; see *Atmospheres* in [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md).
* User Option "3D Render Scale" (setting `render_scale`, new `IVGraphicsManager.render_scale_settings`): the 3D view at 100, 85, 70 or 50% of the window's resolution, upscaled with FSR 1 on Forward+ and bilinear elsewhere, while the GUI keeps full resolution. See *3D render scale* in [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md).
* User Option "Renderer" (setting `renderer`, new `IVGraphicsManager.renderer_settings`), desktop only: Forward+ or Compatibility, written to the file a project names in `application/config/project_settings_override` for the engine to start with next time; a project that names none gets no option. It sits in a new "Graphics (requires restart)" section, and IVOptionsPopup now hides a section with nothing to show and warns while a setting registered through new `IVSettingsManager.set_running_value()` differs from what the running session uses. New `IVGlobal.video_adapter_type` lets a project choose defaults by GPU, in either renderer: Compatibility can't read the GPU's type, so a Forward+ run records it in the same file.
* User Option "Star Catalog" (setting `star_catalog`, new `IVGraphicsManager.star_catalog_settings`) in the "Graphics (requires restart)" section: all 2.6 million catalog stars, or the brightest 940,000 (to V 11) or 220,000 (to V 9.5), which IVStarsVisual loads at the next start. See *The star field* in [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md).
* User Option "Frame Rate Cap" (setting `frame_rate_cap`, new `IVGraphicsManager.frame_rate_cap_settings`): None, 60 or 30 fps, applied live as `Engine.max_fps`; None leaves in place whatever `application/run/max_fps` or `--max-fps` started the engine with. See *Smaller levers* in [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md).
...
* A shadow-mapped IVDynamicLight switches its map off while nothing within its reach would draw into it or read it, under new opt-in `IVCoreSettings.apply_empty_shadow_pass_skip` — renderer-neutral, and worth 27-30 ms of an integrated-GPU Forward+ frame. It is opt-in because each shadowed-light count a session reaches compiles its own programs for every lit shader: see *Local shadow maps* in [VISUAL_MODEL.md](VISUAL_MODEL.md), *The light configuration* in [SHADER_COMPILE_PROFILING.md](SHADER_COMPILE_PROFILING.md) and *Addendum: the empty shadow passes* in [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md). An IVBodyVisual declares itself while it holds `IVGlobal.LOCAL_SHADOW_CASTER`; a project's own local scene does so through new `IVDynamicLight.add_local_shadow_geometry()`.
* [Project breaking] Hi-DPI support: IVGraphicsManager applies the screen's own scale (Windows display scaling, or the browser's devicePixelRatio) as the root window's `content_scale_factor`, so the GUI, body names and symbols, asteroid points and the hover probe keep their size on a hi-DPI screen, a window still at the project's size opens larger to match, and GUI Size scales on top (new `IVCoreSettings.apply_display_scale`, on by default). See *Pixel spaces* in [VISUAL_MODEL.md](VISUAL_MODEL.md). `Viewport.get_visible_rect()` is now logical pixels, so code that took it for render pixels must use new `IVGraphicsManager.get_render_size()`.
* [SHADER_COMPILE_PROFILING.md](SHADER_COMPILE_PROFILING.md): what each shader costs the GPU driver to compile, why the Compatibility renderer (and so the web export) paid 8x what Forward+ does, what a cold start costs now, why hand-unrolling a loop is the expensive case rather than the cheap one, how many programs the light configuration decides a shader compiles — `apply_gl_compatibility_shadows` = false takes a lit shader from four to one, and is the only configuration the warm-up covers completely at its default radii — what a weak part costs through both native GL and Chrome's ANGLE and D3D11 path, where the limb shader probably cannot compile inside Chrome's GPU watchdog at all, and how to measure it again.
* [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md): what each candidate graphics option would buy back on a weak GPU and what it would cost on screen, measured in the [Planetarium](https://github.com/ivoyager/planetarium) on an Intel UHD iGPU and a GTX 1650 Ti — the ranked options, where the frame goes today, the limb shell's dominance of an integrated-GPU frame and the annulus that cut it, the star field and the exposure skips, the free wins that need no option, and the option set it all argues for. Its per-frame figures and [SHADER_COMPILE_PROFILING.md](SHADER_COMPILE_PROFILING.md)'s one-time ones are the two halves of sizing up a weak machine, and each document opens by pointing at the other.

### Changed
* [API breaking] A body with no mesh of its own now picks its shared sphere per frame by on-screen size, from a ladder of rungs halving down from the finest (`IVShellsModel`, built by `IVResourceInitializer.get_sphere_lod_resolutions()`). A rung serves every body whose silhouette sag stays within 0.15 px, so a close body keeps the detail it had while a distant one stops drawing 65,536 triangles into a few pixels. `IVCoreSettings.sphere_radial_segments` and `sphere_rings` are replaced by one `max_sphere_resolution` (256), rings being half the segments at every rung; a project that set either must restate it as the one value. See *The sphere LOD ladder* in [VISUAL_MODEL.md](VISUAL_MODEL.md) and the addendum of the same name in [GRAPHICS_PROFILING.md](GRAPHICS_PROFILING.md), which records why a fixed 128x64 was the wrong answer.
...
* The GUI Size ladder steps down one (`IVCoreSettings.gui_size_multipliers`): Medium, the default, is what Small was (0.75 of the base sizes), Large is what Medium was, and a new smaller Small (0.625) sits below them. A `gui_size` already in a user's settings cache therefore names one size smaller than it did.
