...
* [Asset breaking][API breaking] Saturn's rings ship as one imported `CompressedTexture2DArray` — three half-float `13177 x 1` layers with real mipmaps — in place of 27 PNGs arranged as nine hand-built LOD levels, so `rings.gdshader` reads it with one `textureLod` per layer at a continuous LOD and the hardware's own trilinear filtering replaces a nine-way switch over nine samplers. The texture holds premultiplied linear radiance and composites with `blend_premul_alpha`, which is what Björn Jónsson's profiles have always been: the retired pipeline darkened each radius by its own opacity and then again through an unintended sRGB decode, which is why the C ring, the Cassini Division and the Roche gap had been rendering near-black. `IVAssetPreloader.get_rings_texture_arrays()` becomes `get_rings_texture_array()`, `IVExposureManager.ring_meter_albedo` moves 0.55 -> 0.84 with the level (so the metered B ring barely changes), and rings.gd no longer shares a padding constant with the asset — the texture spans exactly the radii rings.tsv names.
