...
* Tooltips on every user Option (new `IVOptionsPopup.option_tooltips`), with Core's texts in `text/hints_text.csv`. A graphics option's tooltip states its GPU cost for desktop, or for web under the Compatibility renderer (new `option_web_tooltips`).
