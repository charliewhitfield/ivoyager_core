...
* Points and broken dashes along a body's rim wherever the sun stood just above it — the staging that shows it is Jupiter at 0.5 million km with the sun half a radius clear of the limb — because a lit sliver narrower than a pixel was decided by whether that one shading sample happened to fall on the sunlit side. This is the terminator rather than the silhouette, so no MSAA setting could reach it: MSAA resolves coverage and shades a fragment once. The surface, cube-surface, band-pattern and cloud-shell shaders now add the lit fraction of a pixel the terminator crosses (`terminator_lit_excess()` in `shaders/_photometry.gdshaderinc`), as emission, since the engine's own N·L is exactly zero on the dark side of the crossing and no albedo factor can put light where it is missing. Measured on that staging, the rim's longest unbroken run grows by half and the arc carries 60 % more flux; a body the camera resolves at full phase is untouched to the code.
