...
* Three design documents. [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md) is the logic and science of the physical light system: the one-anchor calibration chain, the compensating camera, surface/shell/ring/star/background handling, renderer parity and the settings. [VISUAL_MODEL.md](VISUAL_MODEL.md) is its spatial companion: how a double-precision simulation renders through a float32 pipeline (parenting cancellation, origin shifting, farwarp compression, the ~2^24 near:far ceiling), the shadow systems, culling and lifecycle, orbit-line tiers, point fields and mouse picking. [PHYSICAL_MODEL.md](PHYSICAL_MODEL.md) is the objective simulation underneath both: bodies, orbits as an element coordinate system and why that does not limit thrust or perturbations, trajectories, rotation, time, scale, the small-body groups, the persisted state and what a multiplayer sync would need, and the two kinds of project the model serves. Each carries its own TODO list.
* [IVBody_REDESIGN_v0.3.md](IVBody_REDESIGN_v0.3.md), the living plan for the v0.3 IVBody rework: positioner / rotator / geometry composition, surface anchors, fixed positioners, the proximity service replacing camera-parented sleep and lazy triggers, and a body that answers its own surface geometry. [PHYSICAL_MODEL.md](PHYSICAL_MODEL.md) and [VISUAL_MODEL.md](VISUAL_MODEL.md) link to it where a section describes a v0.2 mechanism it replaces.
* IVShaderWarmup (`program/shader_warmup.gd`), opt-in through `IVCoreInitializer.program_nodes`: it draws spatial shaders on quads in front of the camera, at the planet-scale and the shadow-casting craft-scale layers, so the Compatibility renderer's per-shader compiles land under a loading or splash screen instead of on the first visit to a body. It warms what the project will actually draw rather than every shader in `IVGlobal.resources`: a body's shell shaders come from the specs IVAssetPreloader has already resolved, and Core's remaining shaders from the same conditions IVBodyFinisher and IVSBGFinisher apply when they add the node that binds one. A shader no such condition can decide is not warmed — `stars_shader` is the one Core shader in that position, IVStarsVisual being a scene node — so name it, and any of your own, in `extra_shader_names`; `warm_core_shaders` = false turns the automatic selection off entirely. Its `trigger` suits either boot sequence: `SIMULATOR_STARTED` for a project that boots straight in, where the real scene supplies the specializations bodies use, or `ASSETS_PRELOADED` for a splash-screen project (`wait_for_start` == true), where no system tree exists yet and the warm-up adds its own camera and light to compile the scene-independent part. `progress_changed` and `finished` let the covering screen report it and stay up for it.
* [SHADER_COMPILE_COST.md](SHADER_COMPILE_COST.md): what each shader costs the GPU driver to compile, why the Compatibility renderer (and so the web export) paid 8x what Forward+ does, what a cold start costs now, why hand-unrolling a loop is the expensive case rather than the cheap one, how many programs the light configuration decides a shader compiles — `apply_gl_compatibility_shadows` = false takes a lit shader from four to one, and is the only configuration the warm-up covers completely at its default radii — and how to measure it again.

### Changed
* Updated asset download pointer to v0.2.1.dev.20260901.
* [Asset breaking][API breaking][Table breaking] Saturn's rings are rebuilt end to end. `rings.gdshader` now renders a plane-parallel single-scattering slab, so the rings answer to their opening angle — the optically thin C ring and Cassini Division brighten toward grazing while the saturated B ring holds — and the unlit face renders at all, which it had not: the engine flips a double-sided primitive's normal, so an engine-lit ALBEDO clamped N·L to zero there and the asset's third profile was unreachable (the fragment is self-lit on both renderers now). The asset is one imported `CompressedTexture2DArray` — three half-float `13177 x 1` layers with real mipmaps — in place of 27 PNGs arranged as nine hand-built LOD levels, built by the new `build_saturn_rings.py` in the [tools](https://github.com/ivoyager/tools) submodule; it holds scattering strength with the source's own observing geometry divided out, so the shader re-applies that geometry at the angles it is actually rendering. rings.tsv gains nine photometric columns — the phase function, its opposition surge, the unlit level and the layer's clumping — which is what makes the shader generic to any real or invented ring system, and Saturn's lit-face cells are fitted to its own published magnitude (Mallama & Hilton 2018 give the system and the globe alone as separate equations, so their difference is the rings' flux) rather than set by eye: they reproduce that relation to within 3.8 % from zero phase out to its 6.5 degree limit. Both faces take the same phase function, the scattering angle being 180° − phase whichever side the observer is on; the reference geometry the asset divides out is pinned at the geometry the source images were taken at — Voyager, within 1.5 years of Saturn's 1980 equinox, so a few degrees rather than the ring system's maximum, which roughly doubles the rendered radial contrast and is what a fit cannot supply (it returns a `k` below the one the encounter's own sun elevation demands, because what it absorbs is the radial variation of particle albedo); and the layer's optical depth is modelled as varying across the beam, pinned at the homogeneous limit because nothing in the source profiles constrains it (that fit's R² moves 0.0164 across the whole family against a factor of forty in what the parameter does). `IVAssetPreloader.get_rings_texture_arrays()` becomes `get_rings_texture_array()`, and rings.gd neither shares a padding constant with the asset nor flips the ring plane every frame. Both ring faces now meter: `IVExposureManager` gains `ring_meter_unlit_albedo` beside a re-derived `ring_meter_albedo` (0.55 → 1.04, its meaning now the bright ring's scattering strength) and takes each face's slab term at its own maximum over optical depth, so the two meet at the ring plane instead of switching. What holds that candidate is the ring's own geometry rather than the globe's: its in-frame share is sampled on the annulus itself, a body whose ring or shell reaches outside its own disc is no longer skipped when that disc leaves the frame — which had kept Saturn's rings from metering with the rings filling the frame and the globe panned off the side — and `ring_meter_onset_openness` / `ring_meter_full_openness` hold it by the camera's elevation above the ring plane, the counterpart of the nightside lit-fraction ramp and what turns the handover into a transition rather than a flash (measured with the ramp disabled, screen area alone holds the exposure within half a stop from a 12° opening down to 0.5° and then dumps 13.9 stops between 0.2° and the plane; the ramp spreads the same 17.6 stops over 8° to 1.5°). See [PHOTOMETRIC_MODEL.md](PHOTOMETRIC_MODEL.md). The rings also stop carrying the engine's default specular lobe: a self-lit fragment has to zero `SPECULAR` as well as `ALBEDO`, and without that the engine added an unshadowed F0 = 0.04 reflection on top — Fresnel-amplified toward 1.0 at the grazing angles a ring is seen at, filling the planet's shadow on the rings with a flat grey and compressing the radial contrast everywhere (measured: 0.157 linear, 29% of the lit ansa, on a view at 8.3° elevation). And the unlit face loses the floor it had under it: that value was the source image's own background, flat to 4 % from optical depth 1.4 to 13.8 where single scattering falls a millionfold, so carrying it as a constant made an opaque ring glow — subtracted instead, the unlit face's rendered dynamic range roughly doubles (p90/p10 7.97 → 20.04 at a 12° opening) and its densest B ring goes dark, and the retired `unlit_floor` column takes `unlit_level` with it as a hand-set value, that cell now being derived from the two profiles' measured strength ratio. `IVExposureManager`'s unlit-ring metering also stops scanning for its peak over an optical-depth range the rings overshoot — it stopped at 0.1, where the transmitted term is still climbing, and so read 2.6× low. And the rings stop snapping off as they go edge-on: a fragment's coverage is now the exact share of its own pixel APERTURE on the ring plane that lands on the ring — the pixel's box convolved with the camera's point spread function, `iv_psf_sigma`, on both screen axes, so the ring images through the same Gaussian as every star and every sunlit rim — in place of a first-order radial derivative and a fade on the whole system's minor axis, and the plane is both tilted to stay a few pixels wide — but never past face on, a ring being thin on screen from distance as well as from foreshortening — and expanded outward far enough to hold that aperture's reach, with the ring's light divided by exactly the factor it was thickened. Without the tilt the rasterizer's pixel-centre rule dashes the line and then drops it whole (the lit-pixel count went 996, 682, 408, 0 as the projected minor axis passed 1.5, 1.2, 1.0 and 0.8 pixels); without the aperture a nearly edge-on ring loses a sixth of its flux to sub-pixel sampling and ends in a hard edge that steps a whole row at a time as it closes, which a bloomed hairline makes obvious. `rings.gd` no longer sets an `inner_margin` or an `outer_margin`; the coverage term and the plane's own extent decide where the shader draws.
* [Project breaking] New shader globals `iv_exposure`, `iv_emission_energy_scale`, `iv_emission_luminance_scale`, `iv_limb_scale`, `iv_display_encode` and `iv_psf_sigma`, which the Core editor plugin writes into your project.godot from ivoyager_core.cfg on editor load. Core shaders require these with or without physical light; the default values tell a shader that physical light is inactive, and `iv_psf_sigma` carries IVPSFSettings.psf_sigma to the shaders that image a body's rim.
...
* Shader compile times under the Compatibility renderer, and so the web export, cut 3-7x for the atmosphere and sun shaders: `atmosphere_limb` from 24 s to 4 s, the surface, cloud and band shaders from 11 s to 4 s. The GL compiler was unrolling the constant-bound quadrature, layer and sunspot loops in `_atmosphere.gdshaderinc` and `photosphere.gdshader`; their bounds are now `uniform int`s it cannot see through (`atm_gl6_nodes`, `atm_shell_nodes`, `atm_ring_max_taps`, `spot_cell_reach`), never set. `_photometry.gdshaderinc`'s limb kernel takes the same treatment (`limb_kernel_cells`), for a smaller and unmeasured saving. Renders are unchanged.
