...
* `minnaert_k` on the surface and cube-surface shaders, so a body with a MAP can hold the limb-DARKENED direction at all — the two cloud-deck shaders have carried both laws, while a mapped surface could only ever flatten its disc with `lunar_lambert`. No shipped body sets it and the default is an exact no-op: Uranus and Neptune measure k < 1 and take `lunar_lambert` instead, and Jupiter and Saturn still have no measured value. Note law and level are coupled through the disc factor `F = 2/(2k+1)`, so a body adopting one must re-derive its map level against `A x F = p`.
...
* [IVBodyPSF]'s glare wing is now offset toward the lit limb rather than centred on the body, by a phase-dependent fraction of the silhouette's own radius in the sun's direction, and it carries the flux this camera receives rather than a distant observer's — so it dies with the crescent when a close body's sun sets behind its own disc. A crescent's light is not centred on the body, and with the sun near the limb the cost was plain: the rim is a saturated line, and the one thing that could gradate it sat half a disc away as an even halo. A body at full phase is unchanged, the offset collapsing there on its own.
...
* An atmosphere's beyond-limb ring broke into dashes and isolated coloured pixels as its body shrank on screen — the case that shows is Earth or Titan as a backlit crescent from a few tens of radii out, where the cusps came apart and glow amplified every surviving fragment. The ring's bright band is a few scale heights and so is thinner than a pixel unless the disc spans a couple of hundred px of radius, and where the screen grid put the nearest beyond-limb pixel above that band its light fell instead into a disc pixel, which drew no ring at all. `atm_limb()` now returns the ring averaged over each pixel's own altitude footprint and lets a pixel straddling the silhouette carry its share, reconstructing over rather more than a pixel so the arc has a soft cross-section instead of a one-pixel staircase. The ring's azimuthal roughness at the distances where it showed falls from 6.9 % to 2.8 %, and a resolved limb is untouched.
* Points and broken dashes along a body's rim with the sun within a radius or so of the limb, and the isolated full-brightness pixels that speckled a backlit rim and trailed off a crescent's tips as glow-amplified dashes. With the sun that close the whole lit crescent is a sliver narrower than a pixel, and one shading sample per pixel decided it by where the sample happened to land — the shading RATE is what is short, not coverage, which is why no MSAA setting touched it. The surface, cube-surface, band-pattern and cloud-shell shaders now image each rim pixel through the camera's own PSF (`limb_mean_incidence()` in `shaders/_photometry.gdshaderinc`, on the new `iv_psf_sigma` shader global the star field shares): the sunlight along the pixel's radial reach is integrated in closed form on the sphere's exact parameterization, giving the pixel its true mean sun angle and its coverage by the body, so a rim far above saturation comes out as a continuous line whose width the size law sets, a faintly lit one trails off through intermediate values, and a backlit one gets exactly no light. Coverage replaces the fixed pixel-and-a-half silhouette fade for radiance and alpha — so a crescent or an atmosphere's beyond-limb ring now hands off to the body's distant point source without a step, both ends being the same camera — while specular alone keeps that fade, its grazing lobe being a material question rather than a sampling one. The outward half of that spread, which a surface has no fragment to carry, is drawn by the body's own PSF quad (`limb_sky_side_incidence()`, measured from the exact silhouette conic): a rim that was a knife-edged line — and a staircase wherever its curve stayed within a row — now has the width the size law gives it and an outer falloff to end on. That outward half is drawn only where the body's table figure is what the rasterizer outlines, so Ceres and Charon, which carry their own meshes, get the inward half alone.
