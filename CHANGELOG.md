...
* [IVBodyPSF]'s glare wing is now offset toward the lit limb rather than centred on the body (new `wing_offset` shader uniform; direction the sun's on screen, magnitude growing with phase). A crescent's light is not centred on the body, and with the sun near the limb the cost was plain: the rim is a saturated line, and the one thing that could gradate it sat half a disc away as an even halo. Measured on a Jupiter rim view, arc pixels that are exactly white fall from 46 % to 40 % and the median arc colour moves from near-white to (214, 196, 194); a body at full phase is unchanged to within 16 codes, the offset collapsing there on its own.
...
* Points and broken dashes along a body's rim with the sun within a radius or so of the limb — Jupiter at half a million km with the sun just clear of it, where the whole lit crescent is a sliver half a pixel wide. One shading sample per pixel decided that sliver by where the sample happened to land, so the rim rendered as a dotted chain; the shading RATE is what is short, not coverage, which is why no MSAA setting touched it. The surface, cube-surface, band-pattern and cloud-shell shaders now average the sun angle across each rim pixel's own radial extent (`limb_mean_incidence()` in `shaders/_photometry.gdshaderinc`), using the sphere's exact parameterization rather than a gradient estimate, and counting the part of the filter past the silhouette as the sky it is. Matched against the same view rendered at 2x2 and downsampled: the rim goes from 75 % to 90 % continuous against that reference's 80 %, and a fully backlit rim adds no light at all, which a linearized version of the same idea could not manage.
* Isolated full-brightness pixels speckled a body's limb — tens of them scattered around the rim of a backlit planet, reshuffling as the camera moved, and trailing off the tips of a crescent as dashes that glow then amplified. A silhouette pixel holds a sliver of surface at grazing incidence but gets one shading sample, taken with an interpolated normal that extrapolates past where the geometry turns away, so a fragment that should be unlit reported full sunlight. The surface, cube-surface, band-pattern and cloud-shell shaders now fade a shell's radiance over the last pixel and a half of its silhouette (`limb_fade_factor()` in `shaders/_photometry.gdshaderinc`), measured from the view cosine's own screen gradient so it needs no radius or camera distance and self-scales to any body at any zoom.
